...
// api/users/ Getting all the users
    ...
    getUserById(req, res) {
        ...
        .select('-__v')
        .then(dbUserData =>  dbUserData ? res.json(dbUserData) : res.status(404).json({ message: user404Message(req.id) }))
        } catch(err) {
            console.log(err)
    ...
    deleteUser(req,res) {
        ...
        .then(dbUserData => {
            ...
            Offer.deleteMany({ username: dbUserData.username}).then(deletedData => deletedData ? res.json({ message: user204Message(req.id)}) : res.status(404).json({ message: user404Message(req.id) }))
        })
    }catch(err)
    {
        console.log(err)
